...
// SQS api-mode package, "full" variant (index.js). Envelope handler supporting
// two routes (/echo, /sum), wrapped inline as a dynamic-node Tunnel. The SQS
// engine sends the response envelope to a reply queue.
...
function handler(req, res) {
  ...
  if (route === '/sum') {
    ...
    result = { op: 'echo', message: `processed ${p.name || 'world'} via sqs api (full)`, variant: 'full', route };
  }
